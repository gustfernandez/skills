...
Commands used by `pr-comments` and `pr-finalize`. Copy verbatim — argument shapes are tested.
...
Save as `round_number`. If the result is 0, this is round 1 (the bot just reviewed for the first time). The *next* round (after the user pushes) is `round_number + 1`.

---

## Check for prior decline on same file:line (Phase 0 — re-surfaced decline filter)

Fetch all *resolved* threads, filter to same `path` + `line`, check if first reply starts with `Declined:`.
...
If a match is found, use the prior `body` as the reply for the re-surfaced thread, then resolve the new thread.
...
```bash
-f body='Declined: the field'"'"'s contract requires this shape.'
```
...
Must use GraphQL — REST has no endpoint for this.
...
Use double quotes for the `body` here since we need `$SHORT_SHA` expanded, but only in that variable — keep the fix summary literal.
...
Do NOT run this for `round_number >= 2`. Instead, print the command for the user and let them decide.
...
```bash
# Fails (exit non-zero) if branch has no upstream or is ahead of it
git -C <worktree> status --porcelain=v2 --branch | grep "^# branch.ab" | grep -v "+0"
```

Simpler check: confirm the upstream exists and local HEAD is not ahead:
...
if [ "$LOCAL" != "$REMOTE" ]; then
  echo "Branch not pushed — push first, then re-run /pr-finalize"
  exit 1
fi
```
